...
exports.createIyzicoPayment = functions.region('europe-west1').https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (result.status === 'success') {
            functions.logger.info("Iyzico Payment Success:", { paymentId: result.paymentId, userId: userId });
            return { status: 'success', paymentId: result.paymentId, conversationId: conversationId };
        } else {
            functions.logger.error("Iyzico Payment Failed:", { errorMessage: result.errorMessage, errorCode: result.errorCode, userId: userId });
            throw new functions.https.HttpsError('aborted', result.errorMessage || 'Ödeme sırasında bir hata oluştu.');
        }
    } catch (error) {
        functions.logger.error("Iyzico System Error:", { message: error.message, userId: userId });
